JavaWeb项目 超市订单管理系统
·基于IDEA+MAVEN+SSM框架
·项目环境：IDEA2020.3.1 JDK1.8.0_271 Tomcat9.0.41 MySql5.7.33 Navicat Premium 15
·主目录下sql文件为本项目数据库文件，部署到本地后需在resources文件夹下的db.properties中改变数据库连接配置
·主目录下validatecode的jar包为登录时验证码验证所需环境，若启用需放在项目根目录中且在pom.xml中配置环境成功


## 1.引言

本项目参考自狂神说Java，并基于原项目添加了登录验证码功能validatecode和用户密码修改功能实现。
...
超市管理系统(smbms)作为每个计算机专业的大学生都是一个很好的练手项目，逻辑层次分明，基础功能包括用户的登录和注销，用户和供应商以及订单信息的增删查改的基础功能。可以帮助我们更好的加深理解三层架构的理念，为以后的微服务和作铺垫。
